...
- `simulator.generate_scenario(..., obstacles=None)` / `spawn(..., obstacles=None)`로 건물 등
  장애물을 주입할 수 있다. 각 원소는 `[(x,y),...]` 폴리곤(높이 미상 → 고도 무관 항상 회피) 또는
  `{"polygon":[(x,y),...], "height": h_m}`(높이 있음 → **고도 기반 회피**) 둘 중 하나.
- **고도 기반 회피**: 드론 순항고도가 `건물높이 + 여유고도(기본 15m)`보다 높으면 그 건물은
  회피 대상에서 제외하고 상공으로 직진 통과한다(`pathfinding.plan_path(..., altitude=...)`).
  높이 정보가 없는 건물이나 고도 미상(`altitude=None`)인 경우는 안전하게 항상 회피한다.
